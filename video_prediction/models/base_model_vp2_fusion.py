...
class BaseVideoPredictionModel(object):
    def __init__(self, mode='train', hparams_dict=None, hparams=None,
        ...
        self.inputs = None
        # additional code 1: major
        self.inputs_major = None
        # additional code 2: inferior
        self.inputs_inferior = None
    ...
    def build_graph_major(self, inputs_major):
        self.inputs_major = inputs_major

    def build_graph_inferior(self, inputs_inferior):
        self.inputs_inferior = inputs_inferior
    ...
    def eval_outputs_and_metrics_fn(self, inputs, inputs_major, outputs, num_samples=None,
        ...
        if self.deterministic:
            ...
            def accum_gen_images_and_metrics_fn(a, unused):
                with tf.variable_scope(self.generator_scope, reuse=True):
                    outputs_sample = self.generator_fn(inputs, inputs_major)
...
class VideoPredictionModel(BaseVideoPredictionModel):
    ...
    # self modified
    def tower_fn(self, inputs, inputs_major):
        """
        ...
        # additional code 1: major
        inputs_major = nest.map_structure(transpose_batch_time, inputs_major)
        # additional code 2: inferior
        # inputs_inferior = nest.map_structure(transpose_batch_time, inputs_inferior)
        # additional code 3: major masked inputs
        # major_masked_inputs, inferior_masked_inputs = inputs_inferior, inputs_inferior
        # inputs['images'] * inputs_major['images']
        # additional code 4: inferior masked inputs
        # inputs['images'] * inputs_inferior['images']

        # visualize the major/inferior masked inputs
        # we find that the loaded seqs are shuffle not matched
        vis2 = False
        if vis2:
            sess = tf.Session()
            with sess.as_default():
                [le, bs, width, height, c] = inputs['images'].shape       # seq len, batch size, width, height, channel
                for len_i in range(le):
                    for bs_i in range(bs):
                        # vis_imagei = major_masked_inputs['images'][len_i, bs_i, :, :, :].eval()
                        vis_inputi, vis_inputmi, vis_inputii = inputs['images'][len_i, bs_i, :, :, :].eval(), \
                                                               inputs_major['images'][len_i, bs_i, :, :, :].eval(), \
                                                               inputs_inferior['images'][len_i, bs_i, :, :, :].eval()
                        # visualize inputs * inputs_major
                        vis_masked_major = np.uint8(vis_inputi * vis_inputmi * 255 / np.max(vis_inputi * vis_inputmi))
                        plt.imshow(vis_masked_major)
                        # visualize inputs * inputs_inferior
                        vis_masked_inferior = np.uint8(vis_inputi * vis_inputii * 255 / np.max(vis_inputi * vis_inputii))
                        plt.imshow(vis_masked_inferior)

        with tf.variable_scope(self.generator_scope):
            # gen_outputs = self.generator_fn(inputs)     # should we spatio-temporal learning mask?
            gen_outputs = self.generator_fn(inputs, inputs_major)
            # gen_outputs['gen_images_enc'] = tf.convert_to_tensor(gen_outputs['gen_images_enc'])
            # sess = tf.Session()
            # with sess.as_default():
            #     print(gen_outputs['gen_images_enc'].eval())

        if self.discriminator_fn:
            with tf.variable_scope(self.discriminator_scope) as discrim_scope:
                # discrim_outputs = self.discriminator_fn(inputs, inputs_major, inputs_inferior, gen_outputs)        # self modified for major and infe.. masked
                discrim_outputs = self.discriminator_fn(inputs, inputs_major, gen_outputs)        # self modified for major and infe.. masked
            ...
            with tf.variable_scope(discrim_scope, reuse=True):
                discrim_outputs_post = self.discriminator_fn(inputs, inputs_major, gen_outputs)   # why here? ???  ?  ?     ?  ?    ?
        else:
            discrim_outputs = {}
            discrim_outputs_post = {}
        # 到这里，编码了vae-gan和gan的生成序列和真实序列的各层特征和鉴别网络的ligit逻辑输出。
        ...
        # 把上面得到的多层特征和logit的集合与生成器得到的结果进行结合。
        if self.mode == 'train':
            with tf.name_scope("discriminator_loss"):
                d_losses = self.discriminator_loss_fn(inputs, outputs)      # this has been made by the discriminator_fn, so nothing needs modifying
                print_loss_info(d_losses, inputs, outputs)
            with tf.name_scope("generator_loss"):
                g_losses = self.generator_loss_fn(inputs, inputs_major, outputs)
                ...
                if discrim_outputs_post:
                    ...
                    g_losses_post = self.generator_loss_fn(inputs, inputs_major, outputs_post)
                else:
        ...
        with tf.name_scope("eval_outputs_and_metrics"):
            ...
            eval_outputs, eval_metrics = self.eval_outputs_and_metrics_fn(inputs, inputs_major, outputs)
    ...
    def build_graph(self, inputs, inputs_major):
        # self.inputs = inputs
        BaseVideoPredictionModel.build_graph(self, inputs)
        # additional code 1: major
        BaseVideoPredictionModel.build_graph_major(self, inputs_major)
        # additional code 2: inferior
        # BaseVideoPredictionModel.build_graph_inferior(self, inputs_inferior)
        ...
        if self.num_gpus <= 1:  # cpu or 1 gpu
            ...
            # self modification. add major and inferior as input
            outputs_tuple, losses_tuple, loss_tuple, metrics_tuple = self.tower_fn(self.inputs, self.inputs_major)
            ...
            # here is the code we need to modify for our imp...
            tower_inputs = [OrderedDict() for _ in range(self.num_gpus)]
            # additional 1
            tower_inputs_major = [OrderedDict() for _ in range(self.num_gpus)]
            # additional 2
            # tower_inputs_inferior = [OrderedDict() for _ in range(self.num_gpus)]
            ...
            # additional 1
            for name, input_major in self.inputs_major.items():
                input_splits_major = tf.split(input_major, self.num_gpus)  # assumes batch_size is divisible by num_gpus
                for i in range(self.num_gpus):
                    tower_inputs_major[i][name] = input_splits_major[i]
            # addtional 2
            # for name, input_inferior in self.inputs_inferior.items():
            #     input_splits_inferior = tf.split(input_inferior, self.num_gpus)  # assumes batch_size is divisible by num_gpus
            #     for i in range(self.num_gpus):
            #         tower_inputs_inferior[i][name] = input_splits_inferior[i]
            ...
            for i in range(self.num_gpus):
                ...
                with tf.variable_scope(scope_name, reuse=scope_reuse):
                    with tf.device(device_setter):
                        # self...
                        outputs_tuple, losses_tuple, loss_tuple, metrics_tuple = self.tower_fn(tower_inputs[i], tower_inputs_major[i], tower_inputs_inferior[i])
    ...
    def generator_loss_fn(self, inputs, inputs_major, outputs):
        hparams = self.hparams
        gen_losses = OrderedDict()
        if hparams.l1_weight or hparams.l2_weight or hparams.vgg_cdist_weight:
            # gen_images = outputs.get('gen_images_enc', outputs['gen_images']) * inputs_major['images'][1:]
            # target_images = inputs['images'][1:] * inputs_major['images'][1:]
            gen_images = outputs.get('gen_images_enc', outputs['gen_images'])
            target_images = inputs['images'][1:]
            # gen_images = outputs.get('gen_images_enc', outputs['gen_images']) * inputs_major['images'][1:]
            # target_images = inputs['images'][1:] * inputs_major['images'][1:]
            # visualize gen_images * major and target_images * major
            vis5 = False
            if vis5:
                with tf.Session().as_default():
                    tf.global_variables_initializer().run()
                    [le, bs, width, height, c] = gen_images.shape  # seq len, batch size, width, height, channel
                    for len_i in range(le):
                        for bs_i in range(bs):
                            # take images from tensors. inputs, inputs_major and inputs_inferior
                            vis_img_geni, vis_img_tari = gen_images[len_i, bs_i, :, :, :].eval(), \
                                                         target_images[len_i, bs_i, :, :, :].eval()
                            # visualize images_enc_real major
                            vis_img_geni = np.uint8(vis_img_geni * 255 / np.max(vis_img_geni))
                            plt.imshow(vis_img_geni)
                            # visualize images_enc_fake major
                            vis_img_tari = np.uint8(vis_img_tari * 255 / np.max(vis_img_tari))
                            plt.imshow(vis_img_tari)
            # additional
            # gen_images_add = outputs.get('gen_images', outputs['gen_images_enc']) * inputs_inferior['images'][1:]
            # target_images_add = inputs['images'][1:] * inputs_inferior['images'][1:]
            # gen_images_add = outputs.get('gen_images', outputs['gen_images_enc'])
            # target_images_add = inputs['images'][1:]
            # visualize gen_images * infe and target_images * infe
            vis6 = False
            if vis6:
                with tf.Session().as_default():
                    tf.global_variables_initializer().run()
                    [le, bs, width, height, c] = gen_images_add.shape  # seq len, batch size, width, height, channel
                    for len_i in range(le):
                        for bs_i in range(bs):
                            # take images from tensors. inputs, inputs_major and inputs_inferior
                            vis_img_geni_add, vis_img_tari_add = gen_images_add[len_i, bs_i, :, :, :].eval(), \
                                                         target_images_add[len_i, bs_i, :, :, :].eval()
                            # visualize images_enc_real major
                            vis_img_geni_add = np.uint8(vis_img_geni_add * 255 / np.max(vis_img_geni_add))
                            plt.imshow(vis_img_geni_add)
                            # visualize images_enc_fake major
                            vis_img_tari_add = np.uint8(vis_img_tari_add * 255 / np.max(vis_img_tari_add))
                            plt.imshow(vis_img_tari_add)
            # tensor average, plus operatiton
            # final_gen_images = (gen_images + gen_images_add)/2.0
            # final_target_images = inputs['images'][1:]
            # final_gen_images = (gen_images_add - gen_images) * inputs_inferior['images'][1:]
            # final_gen_images = (gen_images_add - gen_images)
            # final_target_images = inputs['images'][1:] * inputs_inferior['images'][1:]
            # visualize gen_images * infe and target_images * infe
            vis7 = False
            if vis7:
                with tf.Session().as_default():
                    tf.global_variables_initializer().run()
                    [le, bs, width, height, c] = final_gen_images.shape  # seq len, batch size, width, height, channel
                    for len_i in range(le):
                        for bs_i in range(bs):
                            # take images from tensors. inputs, inputs_major and inputs_inferior
                            vis_img_geni_final, vis_img_tari_final = final_gen_images[len_i, bs_i, :, :, :].eval(), \
                                                                 final_target_images[len_i, bs_i, :, :, :].eval()
                            # visualize images_enc_real major
                            vis_img_geni_final = np.uint8(vis_img_geni_final * 255 / np.max(vis_img_geni_final))
                            plt.imshow(vis_img_geni_final)
                            # visualize images_enc_fake major
                            vis_img_tari_final = np.uint8(vis_img_tari_final * 255 / np.max(vis_img_tari_final))
                            plt.imshow(vis_img_tari_final)
        if hparams.l1_weight:
            gen_l1_loss = vp.losses.l1_loss(gen_images, target_images)      # here we should do major and things
            gen_losses["gen_l1_loss"] = (gen_l1_loss, hparams.l1_weight)
            # additional
            # gen_l1_loss_add = vp.losses.l1_loss(gen_images_add, target_images_add)  # here we should do major and things
            # gen_losses["gen_l1_loss_add"] = (gen_l1_loss_add, hparams.l1_weight)
            # average fusion
            # gen_l1_loss_final = vp.losses.l1_loss(final_gen_images, final_target_images)
            # gen_losses["gen_l1_loss_final"] = (gen_l1_loss_final, hparams.l1_weight)
    ...
    def discriminator_loss_fn(self, inputs, outputs):
        ...
        for infix, gan_weight in gan_weights.items():
            if gan_weight:
                discrim_gan_loss_real = vp.losses.gan_loss(outputs['discrim%s_logits_real' % infix], 1.0, hparams.gan_loss_type)    # mse loss on gan-based real
                discrim_gan_loss_fake = vp.losses.gan_loss(outputs['discrim%s_logits_fake' % infix], 0.0, hparams.gan_loss_type)    # mse loss on gan-based fake
                discrim_gan_loss = discrim_gan_loss_real + discrim_gan_loss_fake    # real/fake gan-based loss between the gt videos and generated videos
                discrim_losses["discrim%s_gan_loss" % infix] = (discrim_gan_loss, gan_weight)  # weighted by ...
        vae_gan_weights = {'_image_sn': hparams.image_sn_vae_gan_weight,
                           '_images_sn': hparams.images_sn_vae_gan_weight,
                           '_video_sn': hparams.video_sn_vae_gan_weight}
        for infix, vae_gan_weight in vae_gan_weights.items():
            if vae_gan_weight:
                ...
                discrim_losses["discrim%s_vae_gan_loss" % infix] = (discrim_vae_gan_loss, vae_gan_weight)   # vae-based videos' logit real/fake loss mse+mse
