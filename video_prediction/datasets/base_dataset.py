...
class BaseVideoDataset(object):
    def __init__(self, input_dir, mode='train', num_epochs=None, seed=None,
        ...
        """
        self.input_dir_base = os.path.normpath(os.path.expanduser(input_dir))
        self.mode = mode
        self.num_epochs = num_epochs
        self.seed = seed
        # additional code for same seq shuffle seqs major and inferior
        ...
        if not os.path.exists(self.input_dir_base):
            raise FileNotFoundError("input_dir %s does not exist" % self.input_dir_base)
        self.filenames = None
        self.filenames_major = None
        self.filenames_inferior = None
        # here we should do some modification, if there exists corresponding data, read it.
        # look for tfrecords in input_dir and input_dir/mode directories
        for input_dir in [self.input_dir_base, os.path.join(self.input_dir_base, self.mode)]:
        ...
        if not self.filenames:
            raise FileNotFoundError('No tfrecords were found in %s.' % self.input_dir_base)
        data_mode = 'train'#  additional codes 1: read train_major: which is the masked imgs covering the salient regions
        if self.mode == 'train':
            data_mode = r'train' #data_mode = r'train_major'
        if self.mode == 'test':
            data_mode = r'test_major'
        for input_dir in [self.input_dir_base, os.path.join(self.input_dir_base, data_mode)]:
            filenames_major = glob.glob(os.path.join(input_dir, '*.tfrecord*'))
            if filenames_major:
                self.input_dir_major = input_dir
                self.filenames_major = sorted(filenames_major)  # ensures order is the same across systems
                break
        if not self.filenames_major:
            raise FileNotFoundError('No tfrecords were found in %s.' % os.path.join(self.input_dir_base, data_mode))
        # additional code 2: read train_inferior: which in the (1 - mask) * img
        # data_mode = r'train_inferior'
        # for input_dir in [self.input_dir_base, os.path.join(self.input_dir_base, data_mode)]:
        #     filenames_inferior = glob.glob(os.path.join(input_dir, '*.tfrecord*'))
        #     if filenames_inferior:
        #         self.input_dir_inferior = input_dir
        #         self.filenames_inferior = sorted(filenames_inferior)  # ensures order is the same across systems
        #         break
        # if not self.filenames_inferior:
        #     raise FileNotFoundError('No tfrecords were found in %s.' % os.path.join(self.input_dir_base, data_mode))

        self.dataset_name = os.path.basename(os.path.split(self.input_dir)[0])
        # additional code 1: for major
        self.dataset_name_major = os.path.basename(os.path.split(self.input_dir_major)[0])
        # additional code 2: for inferior
        # self.dataset_name_inferior = os.path.basename(os.path.split(self.input_dir_inferior)[0])
    ...
    def make_dataset(self, batch_size):
        ...
        shuffle = False
        if shuffle:
            self.state = random.getstate()       # for the same shuffle seq.
    ...
    def make_dataset_major(self, batch_size):
        #  self modificatitons, attention same seq. shuffle by matching inputs and labels
        # additional code 1: major
        filenames_major = self.filenames_major
        shuffle = self.mode == 'train' or (self.mode == 'val' and self.hparams.shuffle_on_val)
        shuffle = False
        if shuffle:
            random.setstate(self.state)     # for same seq. shuffle seqs
            random.shuffle(filenames_major)
        # here

        # create a  tfrecord dataset
        # self modification,
        # additional code 1: , major
        dataset_major = tf.data.TFRecordDataset(filenames_major, buffer_size=8 * 1024 * 1024)
        dataset_major = dataset_major.filter(self.filter)
        if shuffle:
            dataset_major = dataset_major.apply(tf.contrib.data.shuffle_and_repeat(buffer_size=1024, count=self.num_epochs))
        else:
            dataset_major = dataset_major.repeat(self.num_epochs)
        ...
        num_parallel_calls = None if shuffle else 1  # for reproducibility (e.g. sampled subclips from the test set)
        # self modification,
        # additonal code 1: major
        dataset_major = dataset_major.apply(tf.contrib.data.map_and_batch(
            _parser, batch_size, drop_remainder=True, num_parallel_calls=num_parallel_calls))
        dataset_major = dataset_major.prefetch(batch_size)

        return dataset_major  # major masked

    # additional code 2: inferior
    def make_dataset_inferior(self, batch_size):
        # additonal code 2: inferior
        filenames_inferior = self.filenames_inferior
        shuffle = self.mode == 'train' or (self.mode == 'val' and self.hparams.shuffle_on_val)
        shuffle = False
        if shuffle:
            random.setstate(self.state)
            random.shuffle(filenames_inferior)
        # here

        # create a  tfrecord dataset
        # additional code 2: inferior
        dataset_inferior = tf.data.TFRecordDataset(filenames_inferior, buffer_size=8 * 1024 * 1024)
        dataset_inferior = dataset_inferior.filter(self.filter)
        if shuffle:
            dataset_inferior = dataset_inferior.apply(
                tf.contrib.data.shuffle_and_repeat(buffer_size=1024, count=self.num_epochs))
        else:
            dataset_inferior = dataset_inferior.repeat(self.num_epochs)
        ...
        num_parallel_calls = None if shuffle else 1  # for reproducibility (e.g. sampled subclips from the test set)
        # additional code 2: inferior
        dataset_inferior = dataset_inferior.apply(tf.contrib.data.map_and_batch(
            _parser, batch_size, drop_remainder=True, num_parallel_calls=num_parallel_calls))
        dataset_inferior = dataset_inferior.prefetch(batch_size)

        return dataset_inferior     # inferior masked
