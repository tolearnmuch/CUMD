...
def main():
    ...
    #                                                                  "the tfrecords")
    # for debug -------------------------------------------------------------------------------------------
    parser.add_argument("--input_dir", type=str, help="either a directory containing subdirectories "
                                                                     "train, val, test, etc, or a directory containing "
                                                                     "the tfrecords")
    # -------------------------------------------------------------------------------------------
    ...
    parser.add_argument("--checkpoint_d", help="directory with checkpoint or checkpoint name (e.g. checkpoint_dir/model-200000)")
    ...
    parser.add_argument("--save_freq", type=int, default=100, help="save frequence of model, 0 to disable")
    ...
    # for debuge ------------------------------------------------------------------------------------------
    """python scripts/train.py --input_dir data/kth 
    --dataset kth 
    --model savp 
    --model_hparams_dict hparams/kth/ours_savp/model_hparams.json 
    --output_dir logs/kth/ours_savp"""
    # -----------------------------------------------------------------------------------------------------
    # args.input_dir = r'G:\video_prediction1\video_prediction\data\kth'
    # args.dataset = r'kth'
    # args.model = r'savp'
    # args.model_hparams_dict = r'G:\video_prediction1\video_prediction/hparams/kth/ours_savp/model_hparams.json'
    # args.output_dir = r'G:\video_prediction1\video_prediction/logs/kth/ours_savp'
    # args.seed = 7
    # # args.resume = True
    # args.checkpoint = r'G:\video_prediction1\video_prediction\logs\kth\ours_savp\model-316000'
    # args.checkpoint_d = r'G:\video_prediction1\video_prediction\logs\kth\ours_savp\model-316000'
    ...
    # here our 1st modification, load additional data for conditional training. may be we should start from here to same seq. shuffle and iterator seq.
    ...
    VideoDataset_major = datasets.get_dataset_class(args.dataset)
    train_dataset_major = VideoDataset_major(
        args.input_dir,
        mode='train',
        hparams_dict=dataset_hparams_dict,
        hparams=args.dataset_hparams)
    # additional code 2: inferior
    # VideoDataset_inferior = datasets.get_dataset_class(args.dataset)
    # train_dataset_inferior = VideoDataset_inferior(
    #     args.input_dir,
    #     mode='train',
    #     hparams_dict=dataset_hparams_dict,
    #     hparams=args.dataset_hparams)
    ...
    # additional code 1: major
    train_tf_dataset_major = train_dataset.make_dataset_major(batch_size)
    train_iterator_major = train_tf_dataset_major.make_one_shot_iterator()
    # additonal code 2: inferior
    # train_tf_dataset_inferior = train_dataset.make_dataset_inferior(batch_size)
    # train_iterator_inferior = train_tf_dataset_inferior.make_one_shot_iterator()

    train_handle = train_iterator.string_handle()
    # additional code 1: major
    train_handle_major = train_iterator_major.string_handle()
    # additional code 2: inferior
    # train_handle_inferior = train_iterator_inferior.string_handle()
    ...
    # additional code 1: major
    iterator_major = tf.data.Iterator.from_string_handle(
        train_handle_major, train_tf_dataset_major.output_types, train_tf_dataset_major.output_shapes)
    inputs_major = iterator_major.get_next()
    # additional code 2: inferior
    # iterator_inferior = tf.data.Iterator.from_string_handle(
    #     train_handle_inferior, train_tf_dataset_inferior.output_types, train_tf_dataset_inferior.output_shapes)
    # inputs_inferior = iterator_inferior.get_next()

    # visualize the inputs, inputs_major and inputs_inferior first.
    vis1 = False
    if vis1:
        sess = tf.Session()
        with sess.as_default():
            [len, bs, width, height, c] = inputs['images'].shape  # seq len, batch size, width, height, channel
            for len_i in range(len):
                for bs_i in range(bs):
                    ...
                    # visualize inputs
                    # mm = np.max(vis_inputi)
                    vis_inputi = np.uint8(vis_inputi * 255)
                    plt.imshow(vis_inputi)
                    # visualize inputs_major
                    vis_inputmi = np.uint8(vis_inputmi * 255 / np.max(vis_inputmi))
                    plt.imshow(vis_inputmi)
                    # visualize inputs_inferior
                    vis_inputii = np.uint8(vis_inputii * 255 / np.max(vis_inputii))
                    plt.imshow(vis_inputii)

    # inputs comes from the training dataset by default, unless train_handle is remapped to the val_handles
    # model.build_graph(inputs, inputs_major, inputs_inferior)
    model.build_graph(inputs, inputs_major)
    ...
    if long_val_dataset is not None:
        ...
        # just repeat the inputs 3 times
        long_model.build_graph(long_val_dataset.make_batch(batch_size), long_val_dataset.make_batch(batch_size))
    else:
        long_model = None
    ...
    max_steps = model.hparams.max_steps         # here may come the problem without notification
    with tf.Session(config=config) as sess:
        print("parameter_count =", sess.run(parameter_count))

        sess.run(tf.global_variables_initializer())
        sess.run(tf.local_variables_initializer())
        model.restore(sess, [args.checkpoint, args.checkpoint_d])
