...
def main():
    """
    results_dir
    ©À©¤©¤ output_dir                              # condition / method
    ©¦   ©À©¤©¤ prediction_eval_lpips_max           # task: best sample in terms of LPIPS similarity
    ©¦   ©¦   ©À©¤©¤ inputs
    ©¦   ©¦   ©¦   ©À©¤©¤ context_image_00000_00.png  # indexed by sample index and time step
    ©¦   ©¦   ©¦   ©¸©¤©¤ ...
    ©¦   ©¦   ©À©¤©¤ outputs
    ©¦   ©¦   ©¦   ©À©¤©¤ gen_image_00000_00.png      # predicted images (only the future ones)
    ©¦   ©¦   ©¦   ©¸©¤©¤ ...
    ©¦   ©¦   ©¸©¤©¤ metrics
    ©¦   ©¦       ©¸©¤©¤ lpips.csv
    ©¦   ©À©¤©¤ prediction_eval_ssim_max            # task: best sample in terms of SSIM
    ©¦   ©¦   ©À©¤©¤ inputs
    ©¦   ©¦   ©¦   ©À©¤©¤ context_image_00000_00.png  # indexed by sample index and time step
    ©¦   ©¦   ©¦   ©¸©¤©¤ ...
    ©¦   ©¦   ©À©¤©¤ outputs
    ...
    dataset_major = VideoDataset_major(
        args.input_dir,
        mode=args.mode,
        num_epochs=args.num_epochs,
        seed=args.seed,
    ...
    # additioanl
    inputs_major = dataset_major.make_batch(args.batch_size)
    input_phs_major = {k: tf.placeholder(v.dtype, v.shape, '%s_ph' % k) for k, v in inputs_major.items()}
    with tf.variable_scope(''):
        model.build_graph(input_phs, input_phs_major)
    ...
    sample_ind = 0
    while True:
        if args.num_samples and sample_ind >= args.num_samples: # sample_ind >= 3: #
            break
        try:
            # major
            input_results = sess.run(inputs)
            # major
            input_results_major = sess.run(inputs_major)
        except tf.errors.OutOfRangeError:
            break
        print("evaluation samples from %d to %d" % (sample_ind, sample_ind + args.batch_size))

        feed_dict = {input_ph: input_results[name] for name, input_ph in input_phs.items()}
        feed_dict_major = {input_ph_major: input_results_major[name] for name, input_ph_major in
                           input_phs_major.items()}
        feed_dict.update(feed_dict_major)
        # major
        # feed_dict_major = {input_ph: input_results[name] for name, input_ph in input_phs.items()}
        # compute "best" metrics using the computation graph
        fetches = {'images': model.inputs['images'], 'images_major': model.inputs_major['images']}
        # major
        # fetches_major = {'images': model.inputs_major['images']}
    ...
    subtasks = ['min', 'avg', 'max']
    for metric_name in metric_names:
        for subtask in subtasks:
            metric_fnames.append(
                os.path.join(output_dir, 'prediction_eval_%s_%s' % (metric_name, subtask), 'metrics', metric_name))
    div_name = 'diversity'
    metric_fnames.append(os.path.join(output_dir, 'prediction_eval_%s' % (div_name), 'metrics', div_name))
